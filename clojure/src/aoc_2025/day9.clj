...
(defn runner*
  [points pred]
  (let [points-alist (mapv vector (range) points)
        point-pairs (for [[i p1] points-alist
                          [j p2] points-alist
                          :when (< i j)]
                      [p1 p2])]
    (->> point-pairs
...
(defn runner
  ...
  [input]
  (runner* (mapv parse-point input) (constantly false)))
...
(defn runner2
  ...
  (let [points (mapv parse-point input)
     ...
     ;; on `(interior-points point-pair)` and `perimeter-points`, since the area
     ;; of the largest rectangles were on the order of 1 billion.)
    (runner* points (fn [point-pair]
